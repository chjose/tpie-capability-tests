...
void writestream(const std::string& fname, size_t n){
  ...
  tpie::file_stream<struct segmentstr_t> out[NUM_STREAMS];

  for(int i=0;i<NUM_STREAMS;i++)
  {
    std::string f = fname+std::to_string(i);
    out[i].open(f, tpie::open::write_only);
    tpie::file_stream<struct segmentstr_t> out[i];
  }

/*
  out.open(fname, tpie::open::write_only);
  tpie::file_stream<struct segmentstr_t> out1;
  out1.open(fname+"1", tpie::open::write_only);
  tpie::file_stream<struct segmentstr_t> out2;
  out2.open(fname+"2", tpie::open::write_only);
  tpie::file_stream<struct segmentstr_t> out3;
  out3.open(fname+"3", tpie::open::write_only);
  tpie::file_stream<struct segmentstr_t> out4;
  out4.open(fname+"4", tpie::open::write_only);
  tpie::file_stream<struct segmentstr_t> out5;
  out5.open(fname+"5", tpie::open::write_only);
  tpie::file_stream<struct segmentstr_t> out6;
  out6.open(fname+"6", tpie::open::write_only);
  */
...
void copystream(const std::string & infile,
  ...
  for(int i=0;i<NUM_STREAMS;i++)
  {
    std::string f = infile+std::to_string(i);
    in[i].open(f);
  }
/*
  in.open(infile);
